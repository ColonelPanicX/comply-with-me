#!/usr/bin/env python3
"""Crawl NIST final publications and download all available PDFs."""
...
BASE_LISTING_URL = "https://csrc.nist.gov/publications/final-pubs"
HEADERS = {"User-Agent": "nist-finals-auto-dl (https://csrc.nist.gov/)"}
REQUEST_TIMEOUT = 30
REQUEST_RETRIES = 3
RETRY_DELAY = 2
...
def fetch_text(session: requests.Session, url: str, *, delay: float) -> Optional[str]:
    ...
    for attempt in range(REQUEST_RETRIES):
        try:
            ...
            logging.warning("Request error (attempt %s) for %s: %s", attempt + 1, url, exc)
...
def parse_listing(html: str, origin: str) -> List[str]:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if not href:
            continue
        if not re.search(r"/pubs/.+/final", href):
            continue
...
def extract_series_number(detail_url: str) -> tuple[str, str]:
    ...
    # Expected pattern: /pubs/<series>/<number>/.../final
    try:
...
def download_one(session: requests.Session, pub: Publication, base_dir: Path) -> tuple[Publication, str, bool]:
    ...
    filename = sanitize_filename(Path(urlparse(pub.download_url).path).name or f"{pub.series}-{pub.number}.pdf")
    ...
    for attempt in range(REQUEST_RETRIES):
        try:
            ...
            logging.warning("Download error (attempt %s) %s: %s", attempt + 1, pub.download_url, exc)
        time.sleep(RETRY_DELAY)
...
def download_all(publications: List[Publication], base_dir: Path, workers: int) -> List[dict]:
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        futures = {
            executor.submit(download_one, session, pub, base_dir): pub for pub in publications if pub.download_url
        }
            ...
            results.append(
                {
                    ...
                    "message": message,
                    "success": success,
                    "path": str((base_dir / pub.series / sanitize_filename(Path(urlparse(pub.download_url or "").path).name))) if success else "",
                }
            )
    return results
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Crawl NIST final publications and download available PDFs."
    )
    parser.add_argument(
        "--max-pages",
        type=int,
...
def main() -> None:
    ...
    downloads_dir = output_dir / "final-pubs"
    ...
    report_path = reports_dir / "download-results.csv"
